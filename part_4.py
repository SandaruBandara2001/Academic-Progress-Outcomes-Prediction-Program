...
while True:
    ...
    f = open("progressionData.txt", "w")        #save to a file
    for item in credits_list:                   #https://stackoverflow.com/questions/41045280/nested-lists-and-saving-into-a-text-file
        f.write('{} {} {}\n'.format(*item))
    f.close()
    ...
    f=open("progressionData.txt", "r")      #load from txt file
    for line in f:
        line=line.strip('\r\n').split('\t')
        for item in line:
            credits_list_retrieved.append(item.split())
    ...
    for i in range(len(credits_list)):        #print progression outcomes
        ...
        print (' -' , credits_list_retrieved[i][0] , ',', credits_list_retrieved[i][1] , ',' , credits_list_retrieved[i][2])
